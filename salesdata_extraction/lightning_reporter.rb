...
results = Watir_harness.run(should_run_headless, class_name, lambda { | log |
  ...
  browser.div(:id, "MainBodyDiv").link(:text, "Publisher Compensation Report").wait_until_present
  browser.div(:id, "MainBodyDiv").link(:text, "Publisher Compensation Report").click
