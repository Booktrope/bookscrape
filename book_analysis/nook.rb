    ...
    begin
      ...
      book_list.each_with_index do | book, index |
        ...
        if book["detailPageUrlNook"].nil? || book["detailPageUrlNook"] == ""
          ...
          Watir_harness.browser.text_field(id: 'searchBarBN').set(bnid)
          #clicking on the search button.
          Watir_harness.browser.input(id: 'searchSubmit').click
        ...
        log.info "looking up the price."
        sleep 0.5
        price = nil
        unless !Watir_harness.browser.ul(id: 'availableFormats').li(:class => 'tab', :class => 'selected').a(class: 'tabPriceSelected').present?
          price = Watir_harness.browser.ul(id: 'availableFormats').li(:class => 'tab', :class => 'selected').a(class: 'tabPriceSelected')
          book_price = price.text.gsub(/\$/, '')
        else
          if Watir_harness.browser.section(:id, "prodPromo").present?
            book_price = Watir_harness.browser.section(:id, "prodPromo").p(:class, 'price').text.gsub(/\$/, '')
          end
        end
        ...
        if Watir_harness.browser.div(:class => "review-status-box").span.exists?
          average_rating = Watir_harness.browser.div(:class => "review-status-box").span.text.strip
        end

        #getting the number of reviews.
        #We use find_elements so selenium wont crash if the book hasn't been rated.
        review_count = 0
        if Watir_harness.browser.div(id: 'prodReviewInfo').a(:class => "gig-rating-readReviewsLink").a.span.exists?
          review_count = Watir_harness.browser.div(id: 'prodReviewInfo').a(:class => "gig-rating-readReviewsLink").text.to_i
        end

        #getting the product image url.
        product_image_url = ""
        product_image_url = Watir_harness.browser.img(:class => "pdpMainImage").src if Watir_harness.browser.img(:class => "pdpMainImage").nil?
